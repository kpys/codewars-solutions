...
function getMostFrequent(json) {
    let lengths = Object.values(json).flat(1)
    let [a,b,c,d,e] = lengths  
    return [a,b,c,d,e].map(e =>{
      const array = e.map(x => e.filter(element => element === x).length)
      const longest = Math.max(...array)
      return e[array.lastIndexOf(longest)]
    });
  }