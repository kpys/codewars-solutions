...
function moreZeros(s){
    s = [...new Set(s)].map(x => x.charCodeAt(0).toString(2))
    let arr = []
    for(let i = 0; i < s.length; i++){
      let zero = s[i].split('').filter(x => x === '0').length
      let one = s[i].split('').filter(x => x === '1').length
      if(zero > one){
        arr.push(s[i])
      }
    }
      return arr.map(x =>String.fromCharCode(parseInt(x, 2)))
  }