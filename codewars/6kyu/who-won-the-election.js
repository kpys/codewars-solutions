...
function getWinner(listOfBallots) {
    ...
    for(let i = 0; i < listOfBallots.length; i++){
      if(obj.hasOwnProperty(listOfBallots[i]) === true){
        obj[listOfBallots[i]] += 1
      } else{
        obj[listOfBallots[i]] = 1
      }
    }
    const values = Math.max(...Object.values(obj))
    const mostVoted = Object.keys(obj).find(key => obj[key] === values)
    return values > listOfBallots.length / 2 ? mostVoted : null
  }