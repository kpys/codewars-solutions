...
Array.prototype.even = function(){
    return this.filter(x => x === +x).filter(x => x % 2 == 0).filter(x => Number.isInteger(x))
  }
  ...
  Array.prototype.under = function(x){
    return this.filter(x => x === +x).filter(element => element < x).filter(x => Number.isInteger(x))
  }
  ...
  Array.prototype.inRange = function(min,max){
    return this.filter(x => x === +x).filter(element => element >= min && element <= max).filter(x => Number.isInteger(x))
  }
